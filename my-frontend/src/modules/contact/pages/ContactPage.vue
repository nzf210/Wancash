  ...
  <div
    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-12 text-center">
        <div class="flex items-center justify-center mb-6">
          ...
          <div class="ml-4">
            ...
            <p class="text-lg text-gray-600 dark:text-gray-300 mt-2">Hubungi kami untuk dukungan token dan staking</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card
            ...
            <CardHeader>
              ...
              <CardDescription class="text-gray-600 dark:text-gray-300">
                Isi formulir di bawah ini dan kami akan membalas dalam waktu 1 jam
              </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <!-- Contact Type -->
                <div class="space-y-2">
                  <Label class="text-gray-700 dark:text-gray-300 font-medium">Jenis Permasalahan</Label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="space-y-2">
                    <Label for="name" class="text-gray-700 dark:text-gray-300">Nama Lengkap</Label>
                    <Input id="name" v-model="form.name" placeholder="Masukkan nama Anda"
                  ...
                  <div class="space-y-2">
                    ...
                    <Input id="email" v-model="form.email" type="email" placeholder="nama@email.com"
                      class="dark:bg-blue-elevated dark:border-blue-light dark:text-white" required />
                  </div>
                </div>
                ...
                <div class="space-y-2">
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">
                    *Opsional, hanya diperlukan untuk masalah teknis terkait transaksi
                  </p>
                </div>

                <div class="space-y-2">
                  <Label for="subject" class="text-gray-700 dark:text-gray-300">Subjek</Label>
                  <Input id="subject" v-model="form.subject" placeholder="Misal: Masalah Staking, Pertanyaan Token, dll"
                    class="dark:bg-blue-elevated dark:border-blue-light dark:text-white" required />
                </div>

                <div class="space-y-2">
                  <Label for="message" class="text-gray-700 dark:text-gray-300">Detail Permasalahan</Label>
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <Label for="urgent" class="text-gray-700 dark:text-gray-300 text-sm cursor-pointer">
                    Tandai sebagai urgent (prioritas tinggi)
                  </Label>
                </div>

                <!-- Attachments -->
                <div class="space-y-2">
                  <Label class="text-gray-700 dark:text-gray-300">Lampiran (Opsional)</Label>
                  <div class="border-2 border-dashed border-gray-300 dark:border-blue-light rounded-lg p-6 text-center">
                    ...
                    <label for="file-upload" class="cursor-pointer">
                      ...
                      <p class="text-gray-600 dark:text-gray-400">Klik untuk upload screenshot atau dokumen</p>
                      <p class="text-sm text-gray-500 dark:text-gray-500 mt-1">Maks. 5MB per file</p>
                    </label>
                  </div>
                  <div v-if="attachments.length > 0" class="mt-4">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">File terlampir:</div>
                    <div class="space-y-2">
        ...
        <div class="space-y-6">
          ...
          <Card class="border-2 border-purple-200 dark:border-blue-light">
            <CardHeader>
              <CardTitle class="text-xl font-bold text-gray-800 dark:text-white">Aksi Cepat</CardTitle>
            </CardHeader>
          ...
          <Card class="border-2 border-purple-200 dark:border-blue-light">
            <CardHeader>
              <CardTitle class="text-xl font-bold text-gray-800 dark:text-white">Kontak Lainnya</CardTitle>
            </CardHeader>
            <CardContent class="space-y-4">
              <div class="flex items-center space-x-3 p-3 rounded-lg bg-gray-50 dark:bg-blue-800/30">
                ...
                <div>
                  <div class="font-semibold text-gray-800 dark:text-white">Telepon Support</div>
          ...
          <Card class="border-2 border-purple-200 dark:border-blue-light">
            <CardHeader>
              <CardTitle class="text-xl font-bold text-gray-800 dark:text-white">FAQ Cepat</CardTitle>
            </CardHeader>
      ...
      <div class="mt-12 bg-gradient-to-r from-purple-500 to-blue-600 rounded-2xl p-8 text-white text-center">
        <h3 class="text-2xl font-bold mb-4">Butuh Bantuan Segera?</h3>
        <p class="mb-6 max-w-2xl mx-auto">Tim support kami siap membantu 24/7. Hubungi kami melalui live chat untuk
          respons instan.</p>
...
// Quick FAQs
const quickFAQs = [
  { id: 1, question: 'Bagaimana cara staking SATKING?' },
  { id: 2, question: 'Berapa minimum deposit untuk staking?' },
  { id: 3, question: 'Apa saja fee yang dikenakan?' },
  { id: 4, question: 'Bagaimana cara claim rewards?' },
  { id: 5, question: 'Apakah token saya aman?' }
]
...
const submitForm = async () => {
  ...
  setTimeout(() => {
    ...
    alert('Pesan Anda telah dikirim! Tim support akan menghubungi Anda dalam waktu 1 jam.')
    resetForm()
    loading.value = false
  }, 1500)
}
  ...
  if (files !== null) {
    for (let i = 0; i < files.length; i++) {
      if (files[i].size <= 5 * 1024 * 1024) { // 5MB limit
        attachments.value.push({ name: files[i].name });
      } else {
        alert(`File ${files[i].name} melebihi ukuran maksimum 5MB`);
      }
    }
  }
}
...
const removeAttachment = (index: number) => {
  if (index >= 0 && index < attachments.value.length) {
    attachments.value.splice(index, 1); // Langsung hapus elemen
  }
};

const connectWallet = () => {
  alert('Fungsi connect wallet akan diimplementasikan di sini')
}

const checkTransaction = () => {
  alert('Fitur cek transaksi akan diimplementasikan di sini')
}

const viewDocumentation = () => {
  alert('Membuka dokumentasi token...')
}

const openLiveChat = () => {
  alert('Membuka live chat support...')
}
</script>
