...
export async function sendEvm(
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            // Re-throw the error if it's not an instance of Error
            throw error
        }
    }
    ...
    const hexBytes = ethers.utils.hexlify(toBytes)
    console.log('Alamat Tujuan: ', hexBytes)
    ...
    try {
        ...
        logger.info('Sending the transaction...', oft.address, sendParam)
        DebugLogger.printErrorAndFixSuggestion(
            KnownErrors.ERROR_QUOTING_NATIVE_GAS_COST,
            `For network-: ${endpointIdToNetwork(srcEid)}, OFT: ${oft.address}`
        )
        throw error
    }
    logger.info('Sending the transaction...', oft.address, sendParam, msgFee)
